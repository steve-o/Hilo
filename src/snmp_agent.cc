...
static const char* kSnmpApplicationName = "Stitch";

LONG volatile hilo::snmp_agent_t::ref_count_ = 0;

class hilo::snmp::event_pump_t
{
public:
...
hilo::snmp_agent_t::snmp_agent_t (hilo::stitch_t& stitch) :
	stitch_ (stitch)
...
bool
hilo::snmp_agent_t::Run (void)
{
/* Instance already running. */
	if (InterlockedExchangeAdd (&ref_count_, 1L) > 0)
		return true;

	if (stitch_.config_.is_agentx_subagent)
	{
		LOG(INFO) << "Configuring as SNMP AgentX sub-agent.";
		if (!stitch_.config_.agentx_socket.empty())
		{
			LOG(INFO) << "Using AgentX socket " << stitch_.config_.agentx_socket << '.';
			::netsnmp_ds_set_string (NETSNMP_DS_APPLICATION_ID,
						 NETSNMP_DS_AGENT_X_SOCKET,
						 stitch_.config_.agentx_socket.c_str());
		}
